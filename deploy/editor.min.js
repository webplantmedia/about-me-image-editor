...
f=b.find(".editor-win-footer").addClass("visible"),$('<button class="button">Apply</button>').appendTo(f).click(function(){var a=document.createElement("canvas");a.width=config.width;a.height=config.height;var b=c.jcrop.scale,d=c.jcrop.crop;a.getContext("2d").drawImage(e(),Math.floor(d.x/b),Math.floor(d.y/b),Math.floor(d.w/b),Math.floor(d.h/b));a=a.toDataURL("image/jpeg",1);this.editor.getSelectedObject().setPhoto(a);this.close()}.bind(this)));this._show(b);d(a)},cropPhotoSave:function(a){var b=$(".editor-win-crop"),
c=this,d,e,g;this.jcrop={};d=function(){return $(".editor-win-crop img").get(0)};0==b.length&&(b=this._create("editor-win-crop","Crop your image"),e=b.find(".editor-win-nav"),e.addClass("editor-win-toolbar"),g=b.find(".editor-win-footer").addClass("visible"),$('<button class="button">Download</button>').appendTo(g).click(function(){var a=document.createElement("canvas");a.width=config.width;a.height=config.height;var b=c.jcrop.scale,e=c.jcrop.crop;a.getContext("2d").drawImage(d(),Math.floor(e.x/b),
Math.floor(e.y/b),Math.floor(e.w/b),Math.floor(e.h/b),0,0,config.width,config.height);a=a.toDataURL("image/png",1);$.ajax({type:"POST",url:config.apiExportURL,data:{imgData:a},cache:!1,success:function(a){$(".export-iframe").remove();$("body").append('<iframe class="export-iframe" src=""></iframe>');$(".export-iframe").attr("src",config.apiExportURL+"?get="+a)}});this.close()}.bind(this)));this._show(b);var f,h;c.jcrop.api&&c.jcrop.api.destroy();f=b.find(".editor-win-contents");f.html("");h=$('<img alt="" />').appendTo(f).get(0);
h.onload=function(){var a=f.width(),b=f.height(),e=getImageSize(d()),a=a/e.width;a*e.height>b&&(a=b/e.height);c.jcrop.scale=a;b=Math.floor(a*e.width);e=Math.floor(a*e.height);h.style.width=b+"px";h.style.height=e+"px";$(h).Jcrop({setSelect:[0,0,b,e],onSelect:function(a){c.jcrop.crop=a}},function(){c.jcrop.api=this})};h.src=a},_build:function(a,b){var c=this,d="editor-win-"+a,e=$("."+d),g,f,h,p,m,s;if(0==e.length)if(e=this._create(d,b),d=e.find(".editor-win-nav"),g=e.find(".editor-win-contents"),"save"==
a)$(".card-save-form").appendTo(g).show();else{var k=$('<ul class="clearfix"></ul>').appendTo(g),j=$('<div class="load-more button">More</div>').appendTo(g),q=$("<h2>Cliparts</h2>"),t=$('<div class="toolbar-sub"></div>').appendTo(d);e.find(".editor-win-head h2").replaceWith(q).wrap('<div class="editor-win-search border-box"></div>');for(var l="",r=0;r<imageEditor.CLIPART_CATEGORIES.length;r++)l+="<span"+("all"===imageEditor.CLIPART_CATEGORIES[r]?' class="active"':"")+">"+imageEditor.CLIPART_CATEGORIES[r]+
"</span>";t.append(l);f=null;d.addClass("editor-win-toolbar");h=null;p=function(b){var e;h=[];b=RegExp(b,"i");for(e=0;e<c.shapes.length;++e)c.shapes[e].type==a&&(-1!=c.shapes[e].tags.search(b)||null!=c.shapes[e].title&&-1!=c.shapes[e].title.search(b))&&(h[h.length]=c.shapes[e])};p("");m=function(){var a=k.find("li").length,b=a+30,c,e;for(b>h.length&&(b=h.length);a<b;++a)c=h[a],e=null!=c.src?'<img src="'+config.assetsThumbURL.format(c.src)+'" alt="" />':c.title,k.append('<li class="thumbnail" data-id="'+
c.id+'"><span class="helper"></span>'+e+"</li>");b>=h.length?j.hide():j.show();$.waypoints("refresh")};j.waypoint(function(){f=$(this);f.waypoint("disable");m();f.waypoint("enable")},{offset:"bottom-in-view",context:g});m();j.on("click",function(){m()});s=function(){"all"===q.val()?p(""):p(q.val());k.html("");k.scrollTop();m()};q.on("input",function(){s()});t.find("span").click(function(){$(".toolbar-sub span").removeClass("active");$(this).addClass("active");q.val($(this).text());s()})}this._show(e);
this._lazyLoad()}});
